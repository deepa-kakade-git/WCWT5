    ...
    if request.method == 'POST':
        ...
        for timesheet_data in timesheets:
            ...
            if is_deleted:
                ...
                dbsession.query(Timesheet).filter_by(TaxpayerInfo_id=taxpayer_id, TimeEntryDate=time_entry_date, TimeEntryCode=time_entry_code).delete()
            else:
                # Otherwise, add/update the timesheet entry in the database
                timesheet = dbsession.query(Timesheet).filter_by(TaxpayerInfo_id=taxpayer_id, TimeEntryDate=time_entry_date, TimeEntryCode=time_entry_code).first()
                if timesheet:
                    ...
                    new_timesheet = Timesheet(
                        ...
                        TimeEntryDate=datetime.datetime.strptime(time_entry_date,'%Y-%m-%d'),
                        TimeEntryCode=time_entry_code,
                        TimeEntryOtherDesc=time_entry_other_desc
                    )
                    app.logger.debug(' Inserting : ' ,new_timesheet.TaxpayerInfo_id,new_timesheet.TimeEntryDate,new_timesheet.TimeEntryCode,new_timesheet.TimeEntryOtherDesc, flush=True)
                    try:
                       dbsession.add(new_timesheet)
                           ...
                           # You can also access other attributes or properties of the SQLAlchemyError object if needed

                           # Log the error message or handle it in an appropriate way
        ...
        taxpayer_info = dbsession.query(Taxpayer_Info).filter_by(id=taxpayer_info_id).first()
        tax_year = taxpayer_info.TaxYear

        # Fetch existing timesheets for the taxpayer from the database
        existing_timesheets = dbsession.query(Timesheet).filter_by(TaxpayerInfo_id=taxpayer_info_id).all()

        # Render the HTML template for adding, displaying, and deleting timesheets
        return render_template('wcwt5_timesheet.html', taxpayer_info_id=taxpayer_info_id, existing_timesheets=existing_timesheets, tax_year=tax_year)
